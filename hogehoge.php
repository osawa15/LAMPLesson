...
try{
    $pdo = new PDO("mysql:host=localhost;dbname=hoge;charset=utf8","root",array(PDO::ATTR_EMULATE_PREPARES => false));
...
if ($item_name && isset($_POST["btn"])){
    ...
    $stmt = $pdo -> prepare("INSERT INTO ahoaho(name,point) VALUES (:name,:point)");
    $stmt->bindParam(':name',$item_name,PDO::PARAM_STR);
    $stmt->bindValue(':point',$item_point,PDO::PARAM_INT);
    $stmt->execute();
}
$select_sql = "SELECT * FROM ahoaho";
